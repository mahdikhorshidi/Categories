...
namespace mahdikhorshidi\categories\Traits;
...
trait Categorizable
{
    ...
    {
        return $this->morphToMany(config('mahdikhorshidi.categories.models.category'), 'categorizable', config('mahdikhorshidi.categories.tables.categorizables'), 'categorizable_id', 'category_id')
                    ->withTimestamps();
    }
     ...
     *
     * @param int|string|array|\ArrayAccess|\mahdikhorshidi\categories\Models\Category $categories
     *
     * @return void
     */
    ...
    protected function prepareCategoryIds($categories): array
    {
        ...
        if (is_string($categories) || (is_array($categories) && is_string(array_first($categories)))) {
            $categories = app('mahdikhorshidi.categories.category')->whereIn('slug', $categories)->get()->pluck('id');
        }
